...
module.exports = { 
    data: new SlashCommandBuilder()
        .setName('remove_role')
        .setDescription('remove role to member')
        ...
        .addStringOption((option) => {
            return option.setName('reason')
                    .setDescription('reason for role removing')}),
    async execute(interaction) {
        ...
        const response = await interaction.reply({
            content: `Are you sure you want to remove ${role} role to ${target} for ${reason}?`,
            components: [row],
        ...
        collector.on('collect', async (i) => {
            if (i.customId === 'confirm') {
                try {
                    await i.update({content: `${role} role has been removed to ${target} for ${reason}.`,components: [], flags: MessageFlags.Ephemeral})
                    await members.removeRole({user: target.id, role: role.id})
